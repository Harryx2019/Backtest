...
class Learner(GetAttr):
    ...
    def _predict(self, pred_data):
        ...
        # self.dl = dl
        self.batch = pred_data
        self.n_inp = 1       # n_inp 控制预测时 batch.yb = None         
        self.model.eval()        #  model at evaluation mode  
        with torch.no_grad(): self.all_batches('predict')        
        self('after_predict')


    def predict(self, pred_data, weight_path=None, Dataset=None, Dataloader=None, batch_size=None):
        """_summary_
        Args:
        ...
        # test_dl = self._prepare_data(pred_data, Dataset, Dataloader, batch_size)
        self._predict(pred_data)  
        self.preds = cb.preds
    ...
    def lr_finder(self, start_lr=1e-7, end_lr=10, num_iter=100, step_mode='exp', show_plot=False, suggestion='valley'):                
        """
